...
FRED-powered indicators require the repository Actions secret `FRED_API_KEY`. Scheduled updates use the official JSON observations API at `api.stlouisfed.org`, request only a 90-day overlap for an existing series, and merge actual published observations into the retained local history. The API key is read only from the environment and is never written to logs or tracked files.
...
| Fed Funds Rate | `data/fed-funds-rate.csv` | FRED `DFEDTAR` through 2008-12-15, then `DFEDTARU` | 1982-09-27 | Official rate changes with FOMC decisions |
| CME Implied Policy Rate | `data/cme-expected-policy-rate.csv`; latest probability detail in `data/fedwatch-expected-rate.json` | CME FedWatch Historical Download | 2026-01-28 | Daily rolling market-implied rate for the next scheduled FOMC meeting, stitched at each FOMC decision date |
| MOVE Index | `data/move.csv` | Google Finance `MOVE:INDEXNYSEGIS` | 2002-11-12 | Daily/business daily |
...
| Nikkei 225 | `data/nikkei-225.csv` | Nikkei Indexes official daily CSV | 1970-01-05 | Daily/Japan trading days |
| TOPIX | `data/topix.csv` | JPX monthly statistics PDFs plus the JPX official current-index file | 2016-01-04 | Daily/Japan trading days |
...
| TAIEX | `data/taiex.csv` | Taiwan Stock Exchange official historical report | 1997-07-02 | Daily/Taiwan trading days |
...
Cloudflare runs the UTC cron expressions from `scheduler/wrangler.jsonc`. Weekdays are written as names because Cloudflare numbers Sunday as day 1. The Worker only calls GitHub's workflow-dispatch API; it never contacts FRED, Google Finance, or another market-data provider. After uploading a Worker version, confirm that the Cron Triggers are visible under the Worker's **Settings > Triggers** page; a deployed script version by itself does not prove that schedules were attached.
